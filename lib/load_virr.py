...
class ReadVirrL1(ReadL1):
        ...
        if self.resolution == 1000:
            ...
            if self.satellite in satellite_type1:
                ...
                if os.path.isfile(lut_file):
                    ...
                    if 'tbb' not in list(lut.keys()):
                        lut['tbb'] = lut_ary[:, 0]
                    if 'rad' not in list(lut.keys()):
                        lut['rad'] = {}

                    for i in range(3, 6):
                        band = 'CH_{:02d}'.format(i)
                        if band not in list(lut['rad'].keys()):
    ...
    def __get_clm_file(self):
        ...
        if self.cloud_file is not None:
            ...
            raise ValueError('云掩码文件不存在: self.cloud_file is None')
