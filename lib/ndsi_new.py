...
def ndsi(i_datetime=None,
    ...
    inn_put_file_l01 = os.path.join(
        aid_path, inn_put_para_path, inn_put_root_l01)
    inn_put_file_l02 = os.path.join(
        aid_path, inn_put_para_path, inn_put_root_l02)
    inn_put_file_l03 = os.path.join(
        aid_path, inn_put_para_path, inn_put_root_l03)
    inn_put_file_l11 = os.path.join(
        aid_path, inn_put_para_path, inn_put_root_l11)
    inn_put_file_l12 = os.path.join(
        aid_path, inn_put_para_path, inn_put_root_l12)
    inn_put_file_l13 = os.path.join(
        aid_path, inn_put_para_path, inn_put_root_l13)
    inn_put_file_l14 = os.path.join(
        aid_path, inn_put_para_path, inn_put_root_l14)
    ...
    if (not year_min <= j_year <= year_max) or (not month_min <= j_month <= month_max) or \
        ...
        raise ValueError(
            'Wrongly Time Setting. Please Retry:{}'.format(i_datetime))
    ...
    ref_bt11um_slope_n = (b_hai_bt11_n - a_low_bt11_n) / \
        (b_hai_t_lat - a_low_t_lat)
    ref_bt11um_slope_s = (b_hai_bt11_s - a_low_bt11_s) / \
        (b_hai_t_lat - a_low_t_lat)
    ...
    index_valid = np.logical_and(
        d_sensor_azimuth > -360, d_sensor_azimuth < 360)
    ...
    index_valid = np.logical_and(
        d_solar_zenith >= 0, d_solar_zenith <= solar_zenith_max)
#     index_valid = np.logical_and(
#         d_solar_zenith > 0, d_solar_zenith < solar_zenith_max)
    index_night = np.logical_and(
        solar_zenith > solar_zenith_max, solar_zenith <= 180)
    night[index_night] = 1
    if index_night.all():
        print('全部是夜晚数据，不处理')
        return
    ...
    # wangpeng modify 20200526
    land_condition = np.logical_or.reduce((i_mask == 1, i_mask == 2))
#     sea_condition = np.logical_or(
#         i_mask == 0, np.logical_and(i_mask > 3, i_mask < 8))

    i_lsm = np.full(data_shape, -1, dtype='i2')
    i_lsm[land_condition] = 1  # 陆地
    i_lsm[~land_condition] = 0  # 海洋
    lsm = i_lsm
    # -------------------------------------------------------------------------
    # GET Cloud Mask
    i_cm = cloud_mask
    ...
    # -------------------------------------------------------------------------
    #  ################# INITIALIZATION
    i_mark = np.zeros(data_shape, dtype=np.uint8)
    i_step = np.zeros(data_shape, dtype=np.uint8)
#     i_available = np.ones(data_shape, dtype=np.int8)
    # wangpeng modify 20200526
    i_available = np.full(data_shape, fill_value=1, dtype=np.uint8)

    # The SOLAR ZENITH ANGLE EXCEEDS MAXIMUM DEG THRESHOLD.
    # 夜晚数据已经是无效值-10了，所以和其他-10统一认为夜晚数据
    index = np.logical_and(a_sunz == -10, True)
    i_mark[index] = 11
    i_step[index] = 3
    i_available[index] = 0

    # The Sensor ZENITH ANGLE EXCEEDS MAXIMUM DEG THRESHOLD.
    index = np.logical_and(a_sata == -10, True)
    i_mark[index] = 11
    i_step[index] = 4
    i_available[index] = 0
...
#     ref_lon[index] = 180
    ...
    # Check status of Location informations.
    index = np.logical_or.reduce(
        (a_suna == -10, a_sunz == -10, a_satz == -10, a_sata == -10,))
    i_mark[index] = 0
    i_step[index] = 7
    i_available[index] = 0

    # wangpeng modify open code commented
    ref_01[ref_01 <= 0.] = 0.
    ref_02[ref_02 <= 0.] = 0.
    ref_03[ref_03 <= 0.] = 0.
    ref_04[ref_04 <= 0.] = 0.
    ...
    if ref_lat_abs is not None:
        ...
        ref_bt11um[idx_] = ref_bt11um_slope_n * \
            np.abs(b_hai_t_lat) + ref_bt11um_offset_n

        idx_ = np.logical_and(index, ref_lat_abs > a_low_t_lat)
        ref_bt11um[idx_] = ref_bt11um_slope_n * \
            np.abs(a_low_t_lat) + ref_bt11um_offset_n

        idx_ = np.logical_and.reduce(
            (index, ref_lat_abs <= a_low_t_lat, ref_lat_abs >= b_hai_t_lat))
        ref_bt11um[idx_] = ref_bt11um_slope_n * \
            ref_lat_abs[idx_] + ref_bt11um_offset_n

        index = ref_lat < 0
        idx_ = np.logical_and(index, ref_lat_abs < b_hai_t_lat)
        ref_bt11um[idx_] = ref_bt11um_slope_s * \
            np.abs(b_hai_t_lat) + ref_bt11um_offset_s

        idx_ = np.logical_and(index, ref_lat_abs > a_low_t_lat)
        ref_bt11um[idx_] = ref_bt11um_slope_s * \
            np.abs(a_low_t_lat) + ref_bt11um_offset_s

        idx_ = np.logical_and.reduce(
            (index, ref_lat_abs >= b_hai_t_lat, ref_lat_abs <= a_low_t_lat))
        ref_bt11um[idx_] = ref_bt11um_slope_s * \
            ref_lat_abs[idx_] + ref_bt11um_offset_s
    else:
        ref_bt11um = None
    ...
    i_tag = np.zeros(data_shape, dtype=np.uint8)
    ...
    if DEBUG:
        ...
        print('i_lsm=', i_lsm[row, col])
        print('cld=', i_cm[row, col])
        ...
        print("i_available", i_available[row, col])
        print('ref_bt11um=', ref_bt11um[row, col])
        print('i_cm=', i_cm[row, col])
        print('i_lsm=', i_lsm[row, col])
    ...
    if no_none((rr_46, ndsi_6, tbb_31)):
        # wangpeng modif 20200529   np.logical_or.reduce ->
        # np.logical_and.reduce
        idx_ = np.logical_and.reduce(
            (rr_46 > 2., ndsi_6 > 0.38, tbb_31 > 274.5))
        idx_ = np.logical_and.reduce((idx_ocean, judge, idx_))
        i_mark[idx_] = 39
        i_step[idx_] = 20
        i_tag[idx_] = 2
        judge[idx_] = False
        # wangpeng modify 20200528
...
#         i_mark[idx_] = 37
    ...
    if no_none((ref_01, ref_02, ref_06, tbb_31)):
        idx_ = np.logical_and.reduce(
            (ref_02 < 11., ref_06 > 4., tbb_31 > 274.5))
        idx_ = np.logical_or.reduce((ref_01 < 7.5, ref_02 < 6., idx_))
        # wangpeng modif 20200529 logical_and -> logical_and.reduce
        idx_ = np.logical_and.reduce((idx_ocean, judge, idx_))
        i_mark[idx_] = 39
        i_step[idx_] = 21
        i_tag[idx_] = 2
...
#         i_mark[idx_] = 37
    ...
    if no_none((ref_lat_abs, tbb_31, ref_26, ref_bt11um)):
        ...
        idx_ = np.logical_and.reduce(
            (ref_lat_abs > 42, ref_lat < 60, tbb_31 < ref_bt11um_copy))
    ...
    if no_none((ref_lon, ref_lat)):
        ...
        idx_ = np.logical_and.reduce(
            (idx_ocean, judge, _condition, _condition2))
        i_mark[idx_] = 50
        i_step[idx_] = 26
        i_tag[idx_] = 2
        judge[idx_] = False
        ...
        idx_ = np.logical_and.reduce(
            (idx_ocean, judge, np.abs(ref_lat) < 30, _condition))
        i_mark[idx_] = 50
        i_step[idx_] = 27
        i_tag[idx_] = 2
        judge[idx_] = False
    ...
    if no_none((ref_06, dt_02, rr_46)):
        idx_ = np.logical_and.reduce(
            (idx_ocean, judge, ref_06 < 6, dt_02 < 5, rr_46 > 3))
        i_mark[idx_] = 39
        i_step[idx_] = 28
        i_tag[idx_] = 2
        judge[idx_] = False
    ...
    if no_none((tbb_31, ndvis)):
        idx_ = np.logical_and.reduce(
            (idx_land, judge, tbb_31 > 278, ndvis > 0.2))
        i_mark[idx_] = 25
        i_step[idx_] = 31
        i_tag[idx_] = 2
        judge[idx_] = False
    ...
    if no_none((dt_12, ndsi_6, dt_01)):
        ...
        # wangpeng modify 20200628
        idx_temp = np.logical_and(dt_12 < -1., ndsi_6 < 0.08)
    ...
    if no_none((rr_46, ref_01, tbb_31)):
        idx_ = np.logical_and.reduce(
            (idx_land, judge, rr_46 > 5.5, ref_01 > 65, tbb_31 > 240.5, tbb_31 < 276.5))
        i_mark[idx_] = 200
        i_step[idx_] = 37
        i_tag[idx_] = 2
        judge[idx_] = False
    ...
    if no_none((ref_dem, ref_06, ref_01, ref_02)):
        idx_ = np.logical_and.reduce(
            (idx_land, judge, ref_dem <= 1800., ref_06 > 28., ref_01 > 34., ref_02 > 44.))
    ...
    if no_none((ref_dem, dt_01)):
        # wangpeng modify ref_dem < 1800
        idx_ = np.logical_and.reduce(
            (idx_land, judge, ref_dem <= 1800, dt_01 > 20.5))
        i_mark[idx_] = 50
        i_step[idx_] = 39
        i_tag[idx_] = 2
        judge[idx_] = False
    ...
    if no_none((ref_dem, ref_06, ref_dem, ref_01, ref_02)):
        ...
        i_tag[idx_] = 2
        judge[idx_] = False
        print("11111111111111111", i_mark[row, col], i_step[row, col])

    if no_none((ref_dem, dt_01)):
        idx_ = np.logical_and.reduce(
            (idx_land, judge, ref_dem >= 1800, dt_01 > (20.5 + (ref_dem - 1800.) * 0.002)))
        i_mark[idx_] = 50
        i_step[idx_] = 41
        i_tag[idx_] = 2
        judge[idx_] = False
    ...
    if no_none((tbb_31, tbb_32, a_satz, ndsi_6)):
        ...
        # wangpeng modify  20200527
#         i_sec_sza[i_sec_sza <= 250] = 0
    ...
    if no_none((ref_dem, ref_bt11um, tbb_31, ref_lat_abs)):
        ...
        compared_t11_hai_lat = np.minimum(
            compared_t11_hai_lat_a, compared_t11_hai_lat_b)

        compared_t11_low_lat_a = ref_bt11um + 12. - ref_dem / 400.
        compared_t11_low_lat_b = 260. - ref_dem / 400.
        compared_t11_low_lat = np.maximum(
            compared_t11_low_lat_a, compared_t11_low_lat_b)

        idx_1 = np.logical_and.reduce(
            (ref_lat_abs >= 40, ref_lat_abs <= 57, tbb_31 < compared_t11_hai_lat))
        idx_2 = np.logical_and.reduce(
            (ref_lat_abs >= 17, ref_lat_abs <= 40, tbb_31 < compared_t11_low_lat))
    ...
    if no_none((ref_dem, ref_26, dt_01, tbb_31, ndsi_6, ref_bt11um)):
        ...
        idx_ = np.logical_or(np.logical_and(ref_26 > compared_ref26, dt_01 > 21),
                             np.logical_and.reduce((ref_26 > compared_ref26 - 7, tbb_31 < ref_bt11um + 8, ndsi_6 > -0.11)))
        # wangpeng modfiy 20200629
        idx_ = np.logical_and.reduce((judge, idx_land, idx_))
        i_mark[idx_] = 50
        i_step[idx_] = 44
        i_tag[idx_] = 2
        judge[idx_] = False
...
#         idx_ = np.logical_or.reduce(
#             (idx_1, idx_2, idx_3, ndsi_6 < -0.21, ndsi_7 < -0.08, dr_16 < -9.8))
#         idx_ = np.logical_and.reduce((idx_land, judge, idx_))
#         i_mark[idx_] = 25
#         i_step[idx_] = 45
#         i_tag[idx_] = 2
#         del idx_1, idx_2, idx_3
    ...
    if no_none((ndvis, ndsi_6, rr_21, dr_16)):
        ...
        idx_ = np.logical_or.reduce(
            (idx_1, idx_2, ndsi_6 < -0.21, dr_16 < -9.5))
        idx_ = np.logical_and.reduce((idx_land, judge, idx_))
        i_mark[idx_] = 25
        i_step[idx_] = 46
        i_tag[idx_] = 2
...
#         judge[idx_] = False
    ...
    # wangpeng modify 20200628   y_ndsi_x_ndvi 101*2  ndsi_6 2030*1353 无法比较
    # 转成一样大小后进行矩阵比较
    def func_ndsi_ndvi(ndvis):
        return y_ndsi_x_ndvi[np.round(ndvis * 100).astype(np.int16), 1]
    vfunc_ndsi_ndvi = np.vectorize(func_ndsi_ndvi)
    ary_y_ndsi_x_ndvi = vfunc_ndsi_ndvi(ndvis)

    if no_none((ndvis, tbb_31, ndsi_6)):
        idx_ = np.logical_and.reduce((ndvis > 0.1, tbb_31 < 277,
                                      ndsi_6 > ary_y_ndsi_x_ndvi))
        idx_ = np.logical_and.reduce((idx_land, judge, idx_))
        i_mark[idx_] = 200
        i_step[idx_] = 47
        i_tag[idx_] = 2
    ...
    if no_none((ndsi_6, ref_lat_abs, tbb_31, ndvis, ref_02, ref_06)):
        ...
        judge[idx_] = False
        del idx_
    # wangpeng modify 20200601  logical_and -> logical_and.reduce
    idx_ = np.logical_and.reduce((idx_land, judge, i_mark == 25))
    judge[idx_] = False
    del idx_
