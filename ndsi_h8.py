...
def ndsi(in_file_l1, in_file_geo):
    ...
    index = np.logical_or.reduce((np.isnan(ref_lon), np.isnan(ref_lat), np.isnan(ref_dem), np.isnan(a_satz),
                                  np.isnan(a_sata), np.isnan(a_sunz), np.isnan(a_suna), np.isnan(lsm)))
    i_mark[index] = 11
    i_step[index] = 1
    i_avalible[index] = 0
    ...
    index = ref_lat >= 0
    if np.abs(ref_lat[index]) < b_hai_t_lat:
        ref_bt11um[index] = ref_bt11um_slope_n * np.abs(b_hai_t_lat) + ref_bt11um_offset_n
    elif np.abs(ref_lat[index]) > a_low_t_lat:
        ref_bt11um[index] = ref_bt11um_slope_n * np.abs(a_low_t_lat) + ref_bt11um_offset_n
    else:
        ref_bt11um[index] = ref_bt11um_slope_n * np.abs(ref_lat[index]) + ref_bt11um_offset_n
    ...
    if np.abs(ref_lat[index]) < b_hai_t_lat:
        ...
        ref_bt11um[index] = ref_bt11um_slope_s * np.abs(ref_lat[index]) + ref_bt11um_offset_s
    ...
    index = np.logical_or.reduce((np.isnan(ref_01), np.isnan(ref_02), np.isnan(ref_03), np.isnan(ref_04),
                                  np.isnan(ref_06), np.isnan(ref_07),
                                  np.isnan(tbb_20), np.isnan(tbb_31), np.isnan(tbb_32),))
    i_mark[index] = 255
    i_step[index] = 2
    i_avalible[index] = 0
    ...
    # !!!========================================================================!!!
        ...
        if judge:
            if tbb_31 < 170 or tbb_31 > 335. or tbb_32 < 170. or tbb_32 > 335. or \
                ...
                i_test_t11 = int(round(tbb_31))
                if i_test_t11 <= 250:
                    i_test_t11 = 250
                if i_test_t11 >= 310:
                    i_test_t11 = 310
                sec_sza = 100. / np.cos(a_satz)
                i_sec_sza = int(round(sec_sza))
                if i_sec_sza >= 250:
                    i_sec_sza = 250
                if test_dtb > delta_bt_lut[int(round(i_test_t11 - 250)), i_sec_sza-100]:
                    i_mark[row, col] = 50
                    i_step[row, col] = 42
                    i_tag = 2
                    judge = False
        ...
        if judge:
            if ndvis > 0.1:
                if ndsi_6 > y_ndsi_x_ndvi[int(round(ndvis * 100)), 1] and \
                        tbb_31 < 277.:
                    i_mark[row, col] = 200
                    i_step[row, col] = 47
                    i_tag = 2
        ...
        if judge:
            if ref_lat > 0:
                ...
                i_nor_s = 1
            if np.abs(r_mon_snow_line[int(round((ref_lon + 180) * 10)), int(j_month), i_nor_s]) > \
                    np.abs(ref_lat) and (i_mark[row, col] == 200. or i_mark[row, col] == 100):
                i_mark[row, col] = 50
                i_step[row, col] = 57
                i_tag = 2
                judge = False
...
def main(in_file):
    if in_file:
        pass
    in_file_l1 = '/DATA2/KTS/CMA_KTS/COM2/H08_L1B/FullDisk/20190513/AHI8_OBI_2000M_NOM_20190513_1000.hdf'
    in_file_geo = '/home/kts/Project/OM/hz/mod_ndsi/Parameters/h8_2km_lut.hdf'
